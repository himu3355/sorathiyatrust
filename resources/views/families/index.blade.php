    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
            ...
            <form action="{{ route('families.index') }}" method="GET" class="flex flex-col md:flex-row gap-4 items-center">
                ...
                <div class="relative w-full flex-grow">
                    ...
                    <input type="text" name="search" value="{{ $searchQuery }}"
                        ...
                        class="w-full pl-12 pr-4 py-3.5 rounded-2xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-emerald-800 bg-slate-50 text-slate-900 placeholder:text-slate-400 font-gujarati text-base shadow-inner">
                </div>
                <div class="flex gap-2 w-full md:w-auto">
                    <button type="submit"
                        class="w-full md:w-auto px-8 py-3.5 bg-emerald-800 hover:bg-emerald-900 text-white font-bold rounded-2xl transition-all shadow-md flex items-center justify-center gap-2">
                ...
                @foreach ($families as $family)
                    <div
                        ...
                        <div class="p-5 border-b border-slate-100 bg-slate-50/70 flex justify-between items-center pl-6">
                            <span
                                class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-100 text-emerald-900 text-xs font-bold border border-emerald-200">
                        ...
                        <div class="p-6 pl-7 flex-grow space-y-4">
                            <div>
                                <h3
                                    class="text-lg font-bold text-slate-900 group-hover:text-emerald-800 transition-colors line-clamp-1 font-gujarati">
                        ...
                        <div class="p-4 pl-7 bg-slate-50/80 border-t border-slate-100">
                            <a href="{{ route('families.show', $family->id) }}"
                                class="w-full py-2.5 px-4 bg-white border border-slate-300 hover:bg-emerald-800 hover:text-white text-slate-800 text-xs font-bold rounded-2xl transition-all shadow-xs flex items-center justify-center gap-2 group/btn">
        ...
        @else
            <div class="bg-white rounded-3xl p-12 text-center text-slate-600 border border-slate-200 shadow-sm space-y-4">
                ...
                <a href="{{ route('families.index') }}"
                    class="inline-flex items-center gap-2 px-6 py-2.5 bg-emerald-800 hover:bg-emerald-900 text-white font-bold text-xs rounded-xl transition-colors">
                    બધા રેકોર્ડ્સ જુઓ
                </a>
            </div>
        @endif
