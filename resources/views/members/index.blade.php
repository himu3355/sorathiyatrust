...
@section('content')
    <x-page-header icon="fa-solid fa-users" title="સમાજ સભ્ય ડિરેક્ટરી (Community Members Directory)"
        subtitle="સમાજના સભ્યો અને ટ્રસ્ટી આગેવાનોની સત્તાવાર યાદી" />

    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <!-- Search & Filter Form -->
        <form action="{{ route('members.index') }}" method="GET"
            class="mb-8 p-4 bg-white rounded-2xl border border-slate-200 shadow-xs flex flex-col sm:flex-row gap-4 items-center">
            ...
            <div class="ml-auto flex gap-2 w-full sm:w-auto">
                ...
                @if (request('search') || request('committee_only'))
                    <a href="{{ route('members.index') }}"
                        class="px-4 py-2.5 bg-slate-200 text-slate-700 font-bold rounded-xl hover:bg-slate-300 transition-colors text-sm">
                        રીસેટ
                    </a>
                @endif
            </div>
                ...
                @foreach ($members as $member)
                    <x-card class="p-6 text-center space-y-4">
                        <div class="w-24 h-24 mx-auto rounded-full overflow-hidden border-4 border-amber-300 shadow-md">
                            ...
                            @else
                                <div
                                    class="w-full h-full gradient-header text-amber-300 font-bold text-2xl flex items-center justify-center">
                        ...
                        <div class="space-y-1">
                            ...
                            @if ($member->designation)
                                <p
                                    class="text-xs font-semibold text-amber-800 font-gujarati bg-amber-50 rounded-lg py-1 px-2 border border-amber-200/60 inline-block">
                                    {{ $member->designation }}
                                </p>
                            @endif
                        </div>
                        ...
                        <div>
                            <a href="{{ route('members.show', $member->id) }}"
                                class="inline-flex items-center justify-center gap-1.5 w-full py-2 bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-bold rounded-xl transition-colors group/btn">
