...
@section('title', ($type === 'upcoming' ? 'આગામી કાર્યક્રમો' : 'ગત કાર્યક્રમો') . ' - શ્રી દશા સોરાઠિયા વાણિયા સમાજ')
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center gap-4 mb-8">
            <div class="flex items-center gap-2 bg-slate-200/80 p-1.5 rounded-xl text-sm font-bold">
                <a href="{{ route('events.upcoming') }}" class="px-5 py-2 rounded-lg transition-all {{ $type === 'upcoming' ? 'bg-emerald-800 text-white shadow-xs' : 'text-slate-700 hover:text-slate-900' }}">
                    આગામી (Upcoming)
                </a>
                <a href="{{ route('events.past') }}" class="px-5 py-2 rounded-lg transition-all {{ $type === 'past' ? 'bg-emerald-800 text-white shadow-xs' : 'text-slate-700 hover:text-slate-900' }}">
                    ગત કાર્યક્રમો (Past)
                </a>
            </div>

            <form action="{{ $type === 'upcoming' ? route('events.upcoming') : route('events.past') }}" method="GET" class="w-full sm:w-auto flex gap-2">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="કાર્યક્રમ શોધો..." class="px-4 py-2 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-emerald-800 bg-white font-gujarati text-sm">
                <button type="submit" class="px-5 py-2 bg-emerald-800 text-white font-bold rounded-xl hover:bg-emerald-900 transition-colors text-sm flex items-center gap-2">
                ...
                @foreach($events as $event)
                    <x-card>
                        ...
                        <div class="p-5 space-y-3">
                            ...
                            <h2 class="text-lg font-bold text-slate-900 font-gujarati line-clamp-2 hover:text-emerald-800 transition-colors">
